...
function Slider({ collection }) {
  ...
  return (
    <Container>
      ...
      <ContainerImages quantity={collection[0]?.length + collection[1]?.length} ref={containerImgs}>
        {collection[1]?.map((item) => (
          <Slide key={item.id}>
            <LinkStyled to={`/tv/${item.id}`} background={`https://image.tmdb.org/t/p/original${item.backdrop_path}`} quantity={collection[0]?.length + collection[1]?.length} />
          </Slide>
        ))}
        {collection[0]?.map((item) => (
          <Slide key={item.id}>
            <LinkStyled to={`/movie/${item.id}`} background={`https://image.tmdb.org/t/p/original${item.backdrop_path}`} quantity={collection[0]?.length + collection[1]?.length} />
          </Slide>
        ))}
      </ContainerImages>
